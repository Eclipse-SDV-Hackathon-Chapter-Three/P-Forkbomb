...
const RESOURCE_LIDAR_SENSOR: u16 = 0x8003;
const RESOURCE_CONTROL_VALUES: u16 = 0x8004;
...
impl UListener for EngageListener {
    async fn on_receive(&self, msg: UMessage) {
        if let Some(payload) = msg.payload {
            // Convert the binary payload to a string
            let value = String::from_utf8(payload.to_vec()).unwrap_or_else(|_| "Invalid UTF-8".to_string());
            log::trace!("[from_uprotocol] engage_status : {}", value);
...
// Listener for lidar sensor data - implements the UListener trait for uProtocol
struct LidarListener {
    // No need to store data, just print it
}
...
impl UListener for LidarListener {
    async fn on_receive(&self, msg: UMessage) {
        ...
        if let Some(payload) = msg.payload {
            // Convert the binary payload to a string (JSON format)
            let value = String::from_utf8(payload.to_vec()).unwrap_or_else(|_| "Invalid UTF-8".to_string());
        } else {
            log::warn!("[LIDAR_LISTENER] Received message with no payload!");
        }
    }
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>>{
    ...
    // Register the lidar sensor listener with uProtocol
    // This listener will be called when lidar sensor messages are received
    let lidar_filter = uri_provider.get_resource_uri(RESOURCE_LIDAR_SENSOR);
    log::info!("=== LIDAR DEBUG ===");
    log::info!("Registering lidar sensor listener [filter: {}]", lidar_filter.to_uri(false));
    log::info!("Lidar filter authority_name: {:?}", lidar_filter.authority_name);
    log::info!("Lidar filter ue_id: {:?}", lidar_filter.ue_id);
    log::info!("Lidar filter resource_id: {:?}", lidar_filter.resource_id);
    log::info!("NOTE: Make sure uprotocol-sensors is started with --ego-vehicle-sensor-lidar-measurement-role=front_lidar");
    
    match transport.register_listener(
        &lidar_filter,
        None,
        Arc::new(LidarListener {}),
    ).await {
        Ok(_) => log::info!("✅ Lidar listener registered successfully!"),
        Err(e) => log::error!("❌ Failed to register lidar listener: {:?}", e),
    }
    log::info!("===================");
    ...
    let control_values_topic = uri_provider.get_resource_uri(RESOURCE_CONTROL_VALUES);   
    ...
    while running.load(Ordering::SeqCst) {
        ...
        if let Some(actor) = carla_world.actor(ego_vehicle_id.unwrap()) {
            if let Ok(ego_vehicle) = actor.into_kinds().try_into_vehicle() {
                ...
                log::info!("Control Values - Throttle: {:.3}, Steer: {:.3}, Brake: {:.3}", throttle, steer, brake);

                // Send control values to PID controller via uProtocol
                let control_payload = format!("{{\"throttle\":{:.3},\"steer\":{:.3},\"brake\":{:.3}}}", throttle, steer, brake);
                log::debug!("[to_uprotocol] control_values : {}", control_payload);
                let control_message = UMessageBuilder::publish(control_values_topic.clone())
                    .build_with_payload(control_payload.clone(), UPayloadFormat::UPAYLOAD_FORMAT_TEXT)?;
                transport.send(control_message).await?;
